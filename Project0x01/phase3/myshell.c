...
/* struct for job management */
typedef struct {
    pid_t pid;          /* Process ID */
    int status;         /* JOB STATUS! 0: running, 1: stopped */
    int state;          /* Job state */
    int id;             /* Job ID */
...
int job_count = 0;   /* Number of jobs in the list */
    ...
    if (!builtin_command(argv, cmdline)) { //quit -> exit(0), & -> ignore, other -> run
        ...
        if (found_pipe) {
            ...
            if (pid == 0) {
                ...
                int status;
                if (!bg) { // If it's a foreground process
...
/* user: Error message print (= perror)*/
...
/* Prints all currently active jobs */
void list_jobs() {
    for (int i = 0; i < job_count; i++) {
        if (jobs[i].pid != 0) {
            printf("[%d] %s %s", i + 1, jobs[i].status ? "Stopped" : "Running", jobs[i].cmdline);
        }
    }
}

/* Changes a stopped background job */
void bg(int job_id) {
    if (job_id > 0 && job_id <= job_count) {
        job_t* job = &jobs[job_id - 1];
        if (job->pid == 0) { // Check if the job has been completed or not.
            printf("No Such Job\n");
        }
        else if (job->status == 1) { // Stopped
            job->status = 0; // Change to running
...
/* Brings a stopped or running background job */
void fg(int job_id) {
    if (job_id > 0 && job_id <= job_count) {
        job_t* job = &jobs[job_id - 1];
        if (job->pid == 0) { // Check if the job has been completed or not.
...
void sigtstp_handler(int sig)
{
    for (int i = 0; i < MAXJOBS; i++) { // Changed from MAXARGS to MAXJOBS
...
void sigint_handler(int sig) {
    ...
    if (pid != 0) {
        kill(-pid, SIGINT);
        printf("Job [%d] (%d) terminated by signal %d\n", pid_to_jid(pid), pid, sig);
    }
}

/* fgpid - Return pid of current foreground job, or 0 if no such job exists */
pid_t fgpid(job_t* jobs) {
    for (int i = 0; i < MAXJOBS; i++) {
        if (jobs[i].state == FOREGROUND) {
            return jobs[i].pid;
